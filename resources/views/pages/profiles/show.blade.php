        ...
        <div class="mx-auto max-w-[1400px] px-4 py-20 md:px-8">
            <div class="flex flex-col items-start gap-6 md:flex-row md:items-center md:justify-between">
                <div class="flex items-center gap-5">
                    ...
                    @else
                        <span
                            class="inline-flex h-20 w-20 items-center justify-center rounded-full prism-bg text-2xl font-black text-white shadow-2xl ring-4 ring-white/20">
                    ...
                    <div>
                        <span
                            class="inline-flex items-center gap-2 rounded-full border border-white/30 bg-white/10 px-3 py-1 text-[11px] font-bold uppercase tracking-widest text-white backdrop-blur">
    ...
    <div class="mx-auto max-w-[1400px] space-y-16 px-4 py-20 md:px-8">
                ...
                <div class="mt-5 flex flex-wrap gap-3">
                    @foreach ($socials as $platform => $url)
                        <a href="{{ $url }}" target="_blank" rel="noopener noreferrer"
                            ...
                            <svg class="h-3.5 w-3.5 text-ink-300 transition group-hover:text-prism-violet"
        ...
        @if ($user->shows('show_collection') && $pinnedCards->isNotEmpty())
            <section>
                <div class="flex items-end justify-between gap-4">
                    ...
                    <span
                        class="shrink-0 rounded-full border border-prism-gold/40 bg-prism-gold/10 px-4 py-1.5 font-mono text-sm font-bold text-ink-900">
        ...
        @if ($user->shows('show_collection'))
            <section>
                ...
                @else
                    <div class="mt-8">
                        <x-empty-state icon="◇" title="No cards pulled yet"
        ...
        @if ($user->shows('show_chase'))
            <section>
                ...
                @else
                    <div class="mt-8">
                        <x-empty-state icon="✦" title="No cards on the hunt"
        ...
        <section>
            ...
            @if (session('status'))
                <div
                    class="mt-5 rounded-2xl border border-prism-mint/40 bg-prism-mint/10 px-4 py-3 text-sm font-semibold text-ink-900">
                    {{ session('status') }}
                </div>
            @endif
                ...
                @auth
                    <form method="POST" action="{{ route('profiles.comment', $user) }}" class="mt-6">
                        ...
                        <div class="rounded-3xl border border-ink-200 bg-white p-4">
                            <textarea name="body" rows="3" maxlength="500" required placeholder="Say something nice…"
            ...
            @else
                <p
                    class="mt-6 rounded-2xl border border-dashed border-ink-200 bg-ink-50 px-4 py-3 text-sm text-ink-500">
            ...
            <div class="mt-8 space-y-4">
                @forelse($comments as $comment)
                    <div class="flex gap-4 rounded-3xl border border-ink-200 bg-white p-5">
                        <span
                            class="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-full prism-bg text-sm font-bold text-white">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-baseline justify-between gap-x-3">
                                ...
                                @endif
                                <span
                                    class="font-mono text-xs text-ink-500">{{ $comment->created_at->diffForHumans() }}</span>
                            </div>
                            ...
                            @auth
                                <div class="mt-3 flex items-center gap-4 text-xs">
                                    @if (auth()->id() == $comment->author_id || auth()->id() == $user->id || auth()->user()->isAdmin())
                                        <form method="POST"
                                            action="{{ route('profiles.comment.destroy', [$user, $comment]) }}"
                                            onsubmit="return confirm('Delete this comment?')">
                                            @csrf @method('DELETE')
                                            <button type="submit"
                                                class="font-semibold text-ink-400 transition hover:text-rose-600">Delete</button>
                                        </form>
                                    @endif
                ...
                @empty
                    <x-empty-state icon="✎" title="No comments yet"
