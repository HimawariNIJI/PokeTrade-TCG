...
<section class="mx-auto max-w-[1200px] px-4 py-12 md:px-8 md:py-16">
    ...
    <div class="grid gap-12 lg:grid-cols-2">
                ...
                <div class="aspect-square bg-gradient-to-br from-ink-50 to-ink-100">
                    ...
                    @else
                        <div class="relative flex h-full items-center justify-center halftone">
                            ...
                            <span class="relative font-display text-6xl font-black text-ink-700/40">{{ strtoupper(substr($item->category, 0, 3)) }}</span>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div>
            <span class="inline-flex items-center gap-2 rounded-full bg-ink-900 px-3 py-1 text-[11px] font-bold uppercase tracking-widest text-white">
                {{ $item->category }}
            </span>
            ...
            <div class="mt-8 rounded-3xl border border-ink-200 bg-white p-6">
                ...
                <form method="POST" action="{{ route('cart.add') }}" class="mt-5">
                    ...
                    <div class="flex gap-3">
                        <input type="number" name="quantity" value="1" min="1" max="{{ max(1, $item->stock) }}"
