    ...
    <form method="POST" action="{{ route('checkout.place') }}" class="grid gap-8 lg:grid-cols-12">
        ...
        <div class="space-y-6 lg:col-span-7">
            <fieldset class="rounded-3xl border border-ink-200 bg-white p-6">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <label class="block">
                        ...
                        <input type="text" name="shipping_name" required value="{{ old('shipping_name', auth()->user()->name) }}"
                    ...
                    <label class="block md:col-span-2">
                        <span class="text-xs font-bold uppercase tracking-widest text-ink-700">Notes (optional)</span>
            ...
            <fieldset class="rounded-3xl border border-ink-200 bg-white p-6">
                ...
                <p class="mb-3 text-xs text-ink-500">Sandbox-only. No real charges. <strong>TODO(team-backend):</strong> wire Midtrans/Stripe sandbox.</p>
                <div class="grid gap-3 md:grid-cols-3">
                    @foreach([
                        ['value' => 'midtrans', 'label' => 'Midtrans (sandbox)', 'sub' => 'GoPay, OVO, BCA VA'],
                        ['value' => 'stripe',   'label' => 'Stripe (test)',     'sub' => '4242 4242 4242 4242'],
                        ['value' => 'cod',      'label' => 'Cash on delivery',  'sub' => 'Pay on arrival'],
                    ] as $i => $pm)
                        <label class="cursor-pointer rounded-2xl border-2 border-ink-200 p-4 hover:border-prism-violet [&:has(input:checked)]:border-ink-900 [&:has(input:checked)]:bg-ink-900 [&:has(input:checked)]:text-white">
                            <input type="radio" name="payment_method" value="{{ $pm['value'] }}" class="sr-only" {{ $i === 0 ? 'checked' : '' }}>
        ...
        <aside class="lg:col-span-5">
            <div class="sticky top-24 space-y-3 rounded-3xl border border-ink-200 bg-white p-6">
                            ...
                            <li class="flex items-center gap-3 py-3">
                                <span class="inline-flex h-12 w-10 shrink-0 overflow-hidden rounded-md bg-ink-100">
                                    @if($it instanceof \App\Models\Card && $it->image_small)
                                        <img src="{{ $it->image_small }}" alt="" class="h-full w-full object-cover">
                                    @endif
                                </span>
                ...
                <a href="{{ route('cart.index') }}" class="block text-center text-xs text-ink-500 hover:text-ink-900">Back to cart</a>
            </div>
        </aside>
    </form>
</section>
