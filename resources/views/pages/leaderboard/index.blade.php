<x-app-layout title="Trainer Leaderboard"
    ...
    <section class="relative isolate overflow-hidden bg-ink-900">
        ...
        <div
            class="pointer-events-none absolute inset-0 -z-20 bg-gradient-to-b from-ink-900/75 via-ink-900/45 to-ink-900/90">
        </div>
    ...
    <section class="mx-auto max-w-[1200px] px-4 py-16 md:px-8" x-data="liveLeaderboard({ url: '{{ route('leaderboard.data') }}', boards: {{ Illuminate\Support\Js::from($boards) }} })" x-init="start()">

        <div class="mb-6 flex items-center justify-end gap-2 text-[11px] font-bold uppercase tracking-widest text-ink-500"
            data-reveal="fade-down">
            <span class="relative flex h-2 w-2">
                <span
                    class="absolute inline-flex h-full w-full animate-ping rounded-full bg-poke-red opacity-75"></span>
        ...
        <div class="grid gap-8 md:grid-cols-3" data-reveal="fade-up" data-reveal-delay="150">
            ...
            <template x-for="board in boards" :key="board.key">
                <div class="rounded-3xl border border-ink-200 bg-white p-6">
                    <div class="mb-5">
                        <p class="text-[10px] font-bold uppercase tracking-widest text-ink-500" x-text="board.eyebrow">
                        </p>
                    ...
                    <template x-if="board.entries.length > 0">
                        <ol class="space-y-2">
                            <template x-for="entry in board.entries" :key="entry.rank">
                                <li>
                                    <a :href="entry.profileUrl" :aria-label="`View ${entry.name}'s profile`"
                                        ...
                                        <span
                                            ...
                                            :class="rankClass(entry.rank)" x-text="entry.rank"></span>
                                        <template x-if="entry.avatar">
                                            <img :src="`/storage/${entry.avatar}`" :alt="`Avatar ${entry.name}`"
                                                class="h-8 w-8 shrink-0 rounded-full object-cover">
                                        </template>

                                        <!-- Jika user TIDAK memiliki avatar (kembali ke inisial huruf) -->
                                        <template x-if="!entry.avatar">
                                            <span
                                        ...
                                        <span class="font-mono text-sm font-bold text-ink-900">
                                            ...
                                            <span class="text-[10px] font-normal text-ink-500"
                                                x-text="board.metricLabel"></span>
                                        </span>
                                    </a>
                                </li>
                            </template>
