...
<section class="relative overflow-hidden bg-ink-900 text-white">
    ...
    <div
        class="mx-auto max-w-[1200px] px-4 py-16 md:px-8 md:py-24"
        x-data="gachaDeck({{ $pulls->count() }})"
        x-cloak
    >
        ...
        <div
            class="gacha-stage relative mx-auto mt-12 w-full select-none"
            x-ref="stage"
            :style="`height:${stageH}px`"
            :class="phase === 'spread' ? '' : 'cursor-pointer'"
            role="button"
        ...
        >
            ...
            @foreach($pulls as $i => $card)
                <div
                    class="gacha-label absolute left-1/2 top-0 text-center"
                    :style="labelStyle({{ $i }})"
                >
                    <p class="line-clamp-1 text-sm font-bold">{{ $card->name }}</p>
                    <p class="text-[10px] uppercase tracking-widest text-white/60">{{ $card->rarity ?? 'Common' }}</p>
                </div>
            @endforeach
        </div>
...
<script>
    ...
    function gachaDeck(total) {
        return {
            ...
            deckW: 240, gap: 14, spreadW: 200, stageH: 335,
