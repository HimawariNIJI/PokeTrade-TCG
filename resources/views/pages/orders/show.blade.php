...
<section class="mx-auto max-w-[1100px] px-4 py-12 md:px-8 md:py-16">
    <a href="{{ route('orders.index') }}" class="inline-flex items-center gap-2 text-xs font-semibold text-ink-500 hover:text-ink-900">
        ← Back to orders
    </a>
            ...
            <div class="rounded-3xl border border-ink-200 bg-white p-7">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-ink-500">Placed {{ $order->created_at->format('M j, Y · g:i A') }}</p>
                    </div>
                    <div class="flex flex-wrap gap-2">
                        <span class="rounded-full px-3 py-1 text-xs font-bold uppercase tracking-widest
                            bg-{{ $order->status_color }}-100 text-{{ $order->status_color }}-700">
                            {{ $order->status }}
                        </span>
                        <span class="rounded-full px-3 py-1 text-xs font-bold uppercase tracking-widest border
                ...
                <ol class="mt-8 grid gap-2 md:grid-cols-4">
                    @foreach(['pending' => 'Order placed', 'paid' => 'Payment confirmed', 'shipped' => 'Shipped', 'delivered' => 'Delivered'] as $key => $label)
                        @php $current = array_search($order->status, ['pending', 'paid', 'shipped', 'delivered']); $i = array_search($key, ['pending', 'paid', 'shipped', 'delivered']); $reached = $current !== false && $i <= $current; @endphp
                        <li class="relative flex items-center gap-3 rounded-2xl border {{ $reached ? 'border-prism-violet bg-white' : 'border-ink-200 bg-ink-50' }} px-4 py-3">
                            <span class="inline-flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold {{ $reached ? 'prism-bg text-white' : 'bg-ink-200 text-ink-500' }}">
                                {{ $loop->iteration }}
                            </span>
                        ...
                        <li class="flex items-center gap-4 py-4">
                            <span class="inline-flex h-16 w-12 shrink-0 overflow-hidden rounded-md bg-ink-100">
                                @if($item->image_snapshot)
                                    <img src="{{ str_starts_with($item->image_snapshot, 'http') ? $item->image_snapshot : asset('storage/' . $item->image_snapshot) }}" alt="" class="h-full w-full object-cover">
                                @endif
                            </span>
