<x-app-layout title="Community Forums"
...
$initialShouts = $shouts
    ...
    <section class="relative isolate overflow-hidden bg-ink-900">
        ...
        <div
            class="pointer-events-none absolute inset-0 -z-10 bg-gradient-to-r from-ink-900/90 via-ink-900/55 to-transparent">
        </div>
        {{-- On mobile the image stacks under the copy, so add a vertical wash for text contrast --}}
        <div
            class="pointer-events-none absolute inset-0 -z-10 bg-gradient-to-b from-ink-900/70 via-transparent to-ink-900/40 md:hidden">
        </div>
        {{-- Bottom fade keeps the seam against the next section soft --}}
        <div
            class="pointer-events-none absolute inset-x-0 bottom-0 -z-10 h-24 bg-gradient-to-t from-ink-900 to-transparent">
        </div>
        ...
        <div class="mx-auto max-w-[1400px] px-4 pb-14 pt-16 md:min-h-[420px] md:px-8 md:pt-20">
            ...
            <div class="mt-7 flex flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <form method="GET" action="{{ route('forums.index') }}" class="relative w-full sm:max-w-md">
                    <svg class="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-white/50"
        ...
        <div class="grid gap-12 lg:grid-cols-12">
            ...
            <div class="lg:col-span-8">
                ...
                @if ($query !== '')
                    <div class="mb-12">
                        <div class="flex items-end justify-between gap-4">
                            <x-section-heading eyebrow="Search"
                                title='Results for <span class="prism-text">{{ e($query) }}</span>' />
                            <a href="{{ route('forums.index') }}"
                                class="shrink-0 text-sm font-bold text-ink-500 hover:text-ink-900">Clear</a>
                        </div>

                        @if ($results->isNotEmpty())
                            <div
                                class="mt-6 divide-y divide-ink-100 overflow-hidden rounded-3xl border border-ink-200 bg-white">
                                @foreach ($results as $thread)
                                    <a href="{{ route('forums.thread', $thread) }}"
                                        class="group flex items-center gap-4 px-5 py-4 transition hover:bg-ink-50">
                                        <div class="min-w-0 flex-1">
                                            <p
                                                class="line-clamp-1 font-display text-sm font-bold text-ink-900 transition group-hover:text-prism-violet">
                                                {{ $thread->title }}</p>
                                            <p class="mt-0.5 line-clamp-1 text-xs text-ink-500">
                                                {{ $thread->author?->name ?? 'Unknown' }}
                                                @if ($thread->category)
                                                    · in <span
                                                        class="font-semibold">{{ $thread->category->name }}</span>
                                                @endif
                                                ·
                                        ...
                                        <span
                                            class="hidden shrink-0 rounded-full bg-ink-100 px-2.5 py-1 font-mono text-[11px] font-bold text-ink-700 sm:inline">
                                            {{ $thread->posts_count }}
                                            {{ Str::plural('reply', $thread->posts_count) }}
                                        </span>
                                    </a>
                                @endforeach
                            </div>
                ...
                <div data-reveal="fade-up">
                    <x-section-heading eyebrow="Boards" title="Browse by <span class='prism-text'>topic</span>" />
                </div>
                        ...
                        @foreach ($categories as $i => $category)
                            ...
                            <a href="{{ route('forums.category', $category) }}"
                                ...
                                <div class="flex items-start justify-between gap-3">
                                    ...
                                    <span
                                        class="shrink-0 rounded-full bg-ink-100 px-2.5 py-1 font-mono text-[11px] font-bold text-ink-700">
                                        {{ $category->threads_count }}
                                        {{ Str::plural('thread', $category->threads_count) }}
                                    </span>
                                </div>
                                ...
                                <div class="mt-4 flex items-center gap-2 text-xs text-ink-500">
                                    @if ($latest)
                                        ...
                                        <span class="line-clamp-1">
                                            Latest: <span
                                                class="font-semibold text-ink-700">{{ $latest->title }}</span>
                                        </span>
                                    @else
                ...
                @else
                    <div class="mt-8">
                        <x-empty-state icon="◇" title="No boards yet"
                ...
                <div class="mt-14">
                    <div data-reveal="fade-up">
                        <x-section-heading eyebrow="Activity"
                        ...
                        <div
                            ...
                            @foreach ($recentThreads as $i => $thread)
                                <a href="{{ route('forums.thread', $thread) }}"
                                    ...
                                    @else
                                        <span
                                            class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full prism-bg text-sm font-bold text-white">
                    ...
                    @else
                        <div class="mt-6">
                            <x-empty-state icon="✦" title="Nothing's been posted yet"
                ...
                <div x-data="shoutbox(@js($initialShouts), @js(auth()->check()), @js(route('shoutbox.index')), @js(route('shoutbox.store')))"
                    ...
                    <div class="relative border-b border-ink-100 px-5 py-4">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <span
                                class="inline-flex items-center gap-1.5 rounded-full bg-prism-mint/15 px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest text-ink-700">
                    ...
                    <div class="flex max-h-[420px] flex-col gap-3 overflow-y-auto px-5 py-4" x-ref="stream">
                        <template x-for="msg in messages" :key="msg.id">
                            <div class="flex items-start gap-3">
                                ...
                                <template x-if="!msg.avatar">
                                    <span
                                        class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full prism-bg text-[11px] font-bold text-white"
                                        x-text="msg.name.charAt(0).toUpperCase()"></span>
                                </template>
                                <div class="min-w-0 flex-1">
                                    <p class="flex items-baseline gap-2">
                                        <span class="font-display text-xs font-bold text-ink-900"
                                            x-text="msg.name"></span>
                        ...
                        <p x-show="messages.length === 0" class="py-6 text-center text-sm text-ink-500">No messages
                            yet. Say hi 👋</p>
                    </div>
                    ...
                    <div class="border-t border-ink-100 p-3">
                        @auth
                            <form @submit.prevent="send()" class="flex items-center gap-2">
                                <input type="text" x-model="draft" maxlength="280" placeholder="Say something nice…"
                                ...
                                <button type="submit" :disabled="sending" aria-label="Send message"
                                    ...
                                    <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                        ...
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M6 12 3.27 3.13a.5.5 0 0 1 .67-.6l16.5 8.5a.5.5 0 0 1 0 .94l-16.5 8.5a.5.5 0 0 1-.67-.6L6 12Zm0 0h8" />
                                    </svg>
                                </button>
                            </form>
                        @else
                            <p class="px-2 py-1 text-center text-[13px] text-ink-500">
                                <a href="{{ route('login') }}"
                                    class="font-semibold text-prism-violet hover:underline">Log in</a> to join the chat.
                            </p>
                        @endauth
                    </div>
                </div>
