<x-admin-layout heading="Auctions" eyebrow="Bidding console">
    <x-slot:actions>
        <x-prism-button :href="route('admin.auctions.create')" size="sm" class="whitespace-nowrap px-3 py-1.5">
            + New Auction
        </x-prism-button>
    </x-slot:actions>
                            ...
                            <tr class="hover:bg-ink-50">
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <p class="font-bold whitespace-nowrap">{{ $auction->card?->name ?? '—' }}</p>
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                                    @php
                                    ...
                                    @endphp
                                    <span class="rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-widest whitespace-nowrap {{ $statusCls }}">
                                        {{ $auction->status }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-right font-mono whitespace-nowrap">@idr($auction->current_bid)</td>
                                <td class="px-4 py-3 text-ink-500 whitespace-nowrap">{{ $auction->currentLeader?->name ?? '—' }}</td>
                                <td class="px-4 py-3">
                                    @if($auction->is_highlighted)
                                        <span class="inline-flex items-center gap-1 rounded-full bg-prism-pink/15 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-widest text-prism-pink whitespace-nowrap">
                                            ⚡ Featured
                                        </span>
                                    @else
                                ...
                                <td class="px-4 py-3 text-xs text-ink-500 whitespace-nowrap">{{ $auction->ends_at?->diffForHumans() }}</td>
