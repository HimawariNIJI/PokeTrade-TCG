...
<div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
    @foreach([
        ['label' => 'Cards in catalog',     'value' => $stats['cards'],     'sub' => 'from API + manual'],
        ['label' => 'Shop items',           'value' => $stats['shop_items'], 'sub' => 'merch & bundles'],
        ['label' => 'Customers',            'value' => $stats['customers'], 'sub' => 'registered users'],
        ['label' => 'Total orders',         'value' => $stats['orders'],    'sub' => $stats['pending'] . ' pending'],
    ] as $s)
...
<div class="mt-8 rounded-3xl border border-ink-200 bg-white">
    <div class="flex items-center justify-between border-b border-ink-100 px-6 py-4">
        ...
        <a href="{{ route('admin.orders.index') }}" class="text-xs font-semibold text-ink-500 hover:text-ink-900">View all →</a>
    </div>
