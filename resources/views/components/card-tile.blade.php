...
<a href="{{ route('cards.show', $card) }}" class="group block">
    <div class="card-surface">
        ...
        @if($card->featured)
            <span class="absolute left-3 top-3 z-10 rounded-full prism-bg px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-white shadow-md">
                ★ Featured
            </span>
        @endif

        @if($card->stock <= 0)
            <span class="absolute right-3 top-3 z-10 rounded-full bg-ink-900/85 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-white">
                Sold out
            </span>
        @elseif($card->stock <= 3)
            <span class="absolute right-3 top-3 z-10 rounded-full bg-amber-500 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-white">
